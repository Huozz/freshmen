...
CARNAMESPACE,ROADNAMESPACE,CROSSNAMESPACE = [],[],[]#只存ID
...
class CROSS(object):
    def __init__(self, id_, north_, east_, south_, west_):
        ...
        self.provider = [self.provider[i][1] for i in range(self.provider.__len__())] #以该路口为出发点的道路号
        # **** dynamic parameters ****#
        self.readyCars = []
        self.leftCars = []
    ...
    def step(self):
        ...
        for index in range(self.provider.__len__()):
            nextCarId.append(ROADDICT[self.provider[index]].firstPriorityCar()) #下一辆要调度的车是这条路上优先级最高的那一辆
