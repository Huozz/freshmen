...
public class Car implements Comparable<Car>{
    ...
    public int getCurRoadId(boolean preset){
        if(!preset)return shortPath.get(curPos);
        return realPath.get(curPos);
    }
    public int getNextRoadId(boolean preset){
        if(!preset)return shortPath.get(curPos+1);
